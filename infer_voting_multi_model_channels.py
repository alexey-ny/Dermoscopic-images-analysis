...
main_logger = myLogger('main', f"{model_logs}{datetime_now}-inf-vote-difficulty-main.log")
res_logger = myLogger('result', f"{model_logs}{datetime_now}-inf-vote-difficulty-results.log", False)
    ...
    for fold in range(n_folds):      
        ...
        for n_ch, ch in enumerate(channels): # running the same model with inputs of just RGB, RGB+EI, RGB+MI, RGB+MI+EI for each fold
            ...
            # with tf.device('/GPU:1'):
                ...
                pred = model.predict(test_gen_NoL, verbose = 1)[:len(test_no_lesion_id_list_IDs)]  
