    ...
    for ch in range (3, params['N_CHANNELS'] + 1): # running the same model with inputs of just RGB, RGB+MI, RGB+MI+EI for each fold
        ...
        with tf.device('/GPU:0'):
            ...
            if n_TTA > 0:
                ...
                if fit_in_RAM:   
                    test_gen = DataGeneratorRAM(test_list_IDs, 'test', labels = None, batch_size = cur_batch_size, img_size = cur_img_size, img_dir = test_image_folder_path, color_space = cur_color_space, testAugment = True, drop_luminosity = drop_luminosity_ch)       
                else:
                    test_gen = DataGenerator(test_list_IDs, labels = None, batch_size = cur_batch_size, img_size = cur_img_size, img_dir = test_image_folder_path, color_space = cur_color_space, testAugment = True, drop_luminosity = drop_luminosity_ch)                      
            ...
            if n_TTA > 0:
                ...
                if fit_in_RAM:   
                    valid_gen = DataGeneratorRAM(valid_list_IDs, 'test-validation', labels = None, batch_size = cur_batch_size, img_size = cur_img_size, img_dir = train_image_folder_path, color_space = cur_color_space, testAugment = True, drop_luminosity = drop_luminosity_ch)
                else:
                    valid_gen = DataGenerator(valid_list_IDs, labels = None, batch_size = cur_batch_size, img_size = cur_img_size, img_dir = train_image_folder_path, color_space = cur_color_space, testAugment = True, drop_luminosity = drop_luminosity_ch)
...
for ch in range(3, params['N_CHANNELS'] + 1):
    ...
    print_log(f'Threshold 0.5 weighted: f1 = {f1[1]:0.2f}, precision = {pr_Score[1]:0.2f}, recall = {recall[1]:0.2f}', [main_logger])
    ...
    if n_TTA > 0:
        ...
        print_log(f'Threshold 0.5 weighted with {n_TTA} TTA: f1 = {f1[1]:0.2f}, precision = {pr_Score[1]:0.2f}, recall = {recall[1]:0.2f}', [main_logger])
...
for ch in [3, 4, 5]:
    ...
    for th in thersholds:
        ...
        # print_log(classification_report(y_test_set, oof_preds[ch] >= th, target_names = ['benign', 'malignant']), [main_logger])
