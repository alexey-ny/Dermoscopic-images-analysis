...
if params['MAL_2019']:
    ...
    print_log('Using malignant samples from 2019', [main_logger, res_logger])
else:
    train_set = train_ext.copy().reset_index(drop = True)
    print_log('Not using malignant samples from 2019', [main_logger, res_logger])
...
for fold, (idxT, idxV) in enumerate(skf.split(train_IP_set)):
    ...
    # for ch in range (3, params['N_CHANNELS'] + 1): # running the same model with inputs of just RGB, RGB+MI, RGB+MI+EI for each fold
        ...
        with tf.device('/GPU:0'):
            ...
            models[ch] = build_model(dim = params['IMG_SIZES'][fold], n_ch = cur_num_channels, net_ind = params['EFF_NETS'][fold], dropout = False, focal_loss = params['FOCAL_LOSS'])
