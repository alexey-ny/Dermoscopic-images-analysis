...
class TestSet():
    ...
    def get_avg_metrics(self, cur_channel):
        preds = self.predictions[cur_channel].mean(axis=0)
...
for n_ch, ch in enumerate(channels): # running the same model with inputs of just RGB, RGB+EI, RGB+MI, RGB+MI+EI for each fold
    ...
    scaler = (test_set_loc.roc_aucs[ch].max() - test_set_loc.roc_aucs[ch].min()+1) 
    weights[ch]= (test_set_loc.roc_aucs[ch] - test_set_loc.roc_aucs[ch].min()/2) / scaler
    ...
    prec, rec, _ = precision_recall_curve(test_set_loc.labels, op >= 0.5)
    auc_precision_recall = auc(rec, prec)
    ...
    print_log(f'{ts:<50} - ROC AUC = {auc_w:.4f}, PR ROC = {auc_precision_recall:.4f}, f1 = {f1[1]:0.4f}, precision = {pr_Score[1]:0.4f}, recall = {recall[1]:0.4f}', [main_logger, res_logger])
    ...
    submission.to_csv(f"{n_folds}_{nets_names[params['EFF_NETS'][0]]}_{params['IMG_SIZES'][0]}_{ch}ch_w_{params['MAL_UPSAMPLE']}ups_{params['COLOR_SPACE']}_CV{'0'+str(auc_w)[2:6]}-ISIC.csv", index=False)
