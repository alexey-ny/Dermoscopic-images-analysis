...
from sklearn.metrics import confusion_matrix, ConfusionMatrixDisplay, precision_recall_fscore_support, precision_recall_curve
...
from plot_utils import plot_mean_ROC, plot_mean_PR, plot_mean_ROC_multi_models, plot_mean_PR_multi_models
...
main_logger = myLogger('main', f"{model_logs}{datetime.now().strftime('%Y%m%d')}-{nets_names[params['EFF_NETS']]}_img{str(params['IMG_SIZES'])}-main.log")
res_logger = myLogger('result', f"{model_logs}{datetime.now().strftime('%Y%m%d')}-{nets_names[params['EFF_NETS']]}_img{str(params['IMG_SIZES'])}-results.log", False)
...
class TestSet():
    ...
    def get_test_gen(self, cur_img_size, cur_channels):
        return DataGenerator(self.list_IDs, self.labels, self.batch_size, cur_img_size, 
                             self.img_dir, self.color_space, self.testAugment, self.drop_luminosity, cur_channels)     
    ...
    def get_last_metrics(self, cur_channel, cur_fold):
        ...
        # print_log(f'### Fold {cur_fold + 1}, {cur_channel} channels, {self.name} - metrics without TTA: ROC AUC = {roc_auc:.4f}, PR AUC = {auc_precision_recall:.4f}, F1 = {f1[1]:.4f}, precision = {precision[1]:.4f}, recall = {recall[1]:.4f}', [main_logger, res_logger])
        # print_log(f'#### Fold {cur_fold + 1}, {cur_channel} channels - metrics without TTA:\n          ROC AUC = {roc_auc:.4f}, PR AUC = {auc_precision_recall:.4f}, F1 = {f1[1]:.4f}, precision = {precision[1]:.4f}, recall = {recall[1]:.4f}', [main_logger, res_logger])
...
# channels = ['RGB', params['COLOR_SPACE'], 'EI', 'MI', '5']

# oof_pred     = {'RGB' : [], '3':[], 'EI':[], 'MI':[], '5':[]}
# oof_pred_TTA = {'RGB' : [], '3':[], 'EI':[], 'MI':[], '5':[]}
...
#              'RGB': np.zeros((len(test_list_IDs), n_folds))}
# preds_TTA = {'3': np.zeros((len(test_list_IDs), n_folds)), 'EI': np.zeros((len(test_list_IDs), n_folds)), 
#              'MI': np.zeros((len(test_list_IDs), n_folds)), '5': np.zeros((len(test_list_IDs), n_folds)),
#              'RGB': np.zeros((len(test_list_IDs), n_folds))}

# oof_aucs     = {'RGB' : [], '3':[], 'EI':[], 'MI':[], '5':[]}
# oof_aucs_TTA = {'RGB' : [], '3':[], 'EI':[], 'MI':[], '5':[]}
...
test_set_loc = TestSet(test_list_IDs, labels = y_test, batch_size = cur_batch_size, img_size = cur_img_size, 
...
test_set_Kaggle = TestSet(test_Kaggle_list_IDs, labels = None, batch_size = cur_batch_size, img_size = cur_img_size, 
...
test_set_NoL = TestSet(test_no_lesion_id_list_IDs, labels = y_test_no_lesion, batch_size = cur_batch_size, img_size = cur_img_size, 
...
for fold, (idxT, idxV) in enumerate(skf.split(train_IP_set)):
    ...
    # for ch in range (3, params['N_CHANNELS'] + 1): # running the same model with inputs of just RGB, RGB+MI, RGB+MI+EI for each fold
        ...
        with tf.device('/GPU:0'):
            ...
            n_TTA = params["TTA"]
            test_gen =          test_set_loc.get_test_gen(cur_img_size, ch)
            test_gen_Kaggle =   test_set_Kaggle.get_test_gen(cur_img_size, ch)
            test_gen_NoL =      test_set_NoL.get_test_gen(cur_img_size, ch)

            # Predict 3 tests without USING TTA
...
with open(f"{n_folds}_{nets_names[params['EFF_NETS']]}_{params['IMG_SIZES']}_{ch}ch_w_{n_TTA}TTA_{params['MAL_UPSAMPLE']}ups_{params['COLOR_SPACE']}_CV{'0'+str(auc_w)[2:6]}.params", 'wb') as f: 
...
#     oof_ch = oof_preds[ch]
#     op = MM_scaler.fit_transform(oof_preds_w[ch].reshape(-1, 1))
#     for th in thersholds:f'{nets_names[params["EFF_NETS"][0]]}-full-ISIC', to_dir = f'{inferences}/{nets_names[params["EFF_NETS"][0]]}')
#         fig, ax = plt.subplots()
...
plot_mean_ROC(y_test_set, oof_pred, channels, model_descr = f'{nets_names[params["EFF_NETS"]]}-full-ISIC', to_dir = f'{inferences}/{nets_names[params["EFF_NETS"]]}')
plot_mean_PR(y_test_set, oof_pred, channels, model_descr = f'{nets_names[params["EFF_NETS"]]}-full-ISIC', to_dir = f'{inferences}/{nets_names[params["EFF_NETS"]]}')
# plot_mean_ROC(y_test_set, oof_pred, [3, 4, 5], model_descr = f'{n_models}-models-combined-{len(tests)}-sets', to_dir = f'{inferences}/{m}')
# plot_mean_PR(y_test_set, oof_pred, [3, 4, 5], model_descr = f'{n_models}-models-combined-{len(tests)}-sets', to_dir = f'{inferences}/{m}')
# plot_mean_ROC_multi_models(y_test_set, oof_pred, [3, 4, 5], model_descr = f'{n_models}-models-combined-{len(tests)}-sets', to_dir = f'{inferences}/{m}')
# plot_mean_PR_multi_models(y_test_set, oof_pred, [3, 4, 5], model_descr = f'{n_models}-models-combined-{len(tests)}-sets', to_dir = f'{inferences}/{m}')
