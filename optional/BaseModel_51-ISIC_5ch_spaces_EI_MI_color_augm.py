...
from tensorflow.keras.layers import Input, GlobalAveragePooling2D, Dropout, Dense, Conv2D, BatchNormalization, MaxPooling2D
...
#  resume after a system crash - will restore certain state, and skip finished folds
f_resume = True
# f_resume = False

if f_resume:
    datetime_now = '20240202' # what was the date on which the model started?
else:
...
params = {
    ...
    'FIT_SKIN' : False, # normalize images for skin tones range
    # 'FIT_SKIN' : True, # normalize images for skin tones range
    'LR_START'  : 0.000005,
    'LR_MAX'    : 0.00000125,
    # 'LR_MIN'   : 0.00005,  
    # 'LR_START'  : 0.0005,
...
if DEVICE != "TPU":
    ...
    # if MULTI:
        ...
        try:
          ...
          tf.config.set_visible_devices(physical_devices_GPU[params['GPU']], 'GPU')
          tf.config.set_logical_device_configuration(
               physical_devices_GPU[params['GPU']],
        ...
        # if gpu_devices:
            for gpu_device in logical_devices_GPU:
                print_log('device available:' + str(logical_devices_GPU), [main_logger])
...
df_combo_test = pd.concat([df_test, df_test_no_lesion_id]).reset_index(drop = True)
...
y_test_combo = df_combo_test['target']
print_log(f'Combo test set (15% full ISIC + NoL) target classes counts: \n{y_test_combo.value_counts()}', [main_logger])
...
def get_lr_callback_kaggle(batch_size=8):
    lr_start   = params['LR_START']
    lr_max     = params['LR_MAX'] * REPLICAS * batch_size
    lr_min     = params['LR_MIN']
    lr_ramp_ep = params['LR_RAMP_EP']
    lr_sus_ep  = params['LR_SUS_EP']
    lr_decay   = params['LR_DECAY']
...
def build_model_simple(dim = 128, n_ch = 3, net_ind = 0, dropout = False, focal_loss = False, PrRec_metric = False): 
# def build_model(dim = 128, n_ch = 3, net_ind = 0, dropout = False, focal_loss = False): 
    inp = Input(shape = (dim, dim, n_ch), name = 'Image')
    # x = tf.cast(inp, tf.float32)
    # if net_ind < 8:
    ...
    # base = EFNS[net_ind](input_shape = (dim, dim, n_ch), weights='imagenet', include_top = False)
    # base = EFNS[net_ind](input_shape = (dim, dim, n_ch), weights = None, include_top = False)
    x = Conv2D(filters = 32, kernel_size = 3)(inp)
    x = MaxPooling2D()(x)
...
#---------------------------------------------------------------------------------------------
def fit_skin(img):
    # np_img = np.array(img * 255).astype(int)
    ...
    if red_max > 255:
        np_img[:, :, 0] = (np_img[:, :, 0]/red_max * 255).astype(np.uint8)
    if green_max > 255:
        np_img[:, :, 1] = (np_img[:, :, 1]/green_max * 255).astype(np.uint8)
    if blue_max > 255:
        np_img[:, :, 2] = (np_img[:, :, 2]/blue_max * 255).astype(np.uint8)
...
#---------------------------------------------------------------------------------------------
def read_img(path, desired_size, color_space = 'RGB', toAugment = False, drop_luminosity = False, sel_channels = '3', skin_fit = False):
    """
    ...
    # fit image for skin tones range
    if skin_fit:
        img_BGR = fit_skin(img_BGR)
        ...
        if self.labels is not None: # training phase
            ...
            for i, ID in enumerate(list_IDs_temp):
                X[i,] = read_img(self.img_dir+ID+".jpg", self.img_size, self.color_space, toAugment = True, 
                                 drop_luminosity = self.drop_luminosity, sel_channels = self.sel_channels, skin_fit = self.skin_fit)
...
class TestSet():
    ...
    def get_test_gen(self, cur_img_size, cur_color_space, cur_channels, skin_fit = False):
        return DataGenerator(self.list_IDs, self.labels, self.batch_size, cur_img_size, 
                             self.img_dir, cur_color_space, self.testAugment, self.drop_luminosity, cur_channels, skin_fit)     
...
test_combo_list_IDs = df_combo_test.isic_id
...
test_set_combo = TestSet(test_combo_list_IDs, labels = y_test_combo, batch_size = cur_batch_size, img_size = cur_img_size, channels = channels,
                         img_dir = test_no_lesion_id_image_folder_path, color_space = cur_color_space, testAugment = False, 
                         drop_luminosity = drop_luminosity_ch, name = 'Combo Test')                      

if f_resume:
    #  set this manually when needed aftr a crash
    # skip_folds = [0, 1, 2, 3, 4] # to skip folds after a potential system crash
    skip_folds = [0, 1, 2, 3] # to skip folds after a potential system crash
else:
    skip_folds = [] 

save_name_temp = f"{n_folds}_{nets_names[params['EFF_NETS']]}_{params['IMG_SIZES']}_{params['BATCH_SIZES']}_w_{n_TTA}TTA_{params['MAL_UPSAMPLE']}ups_{params['COLOR_SPACE']}_{datetime_now}"
# save_name_temp = f"{n_folds}_{nets_names[params['EFF_NETS']]}_{params['IMG_SIZES']}_w_{n_TTA}TTA_{params['MAL_UPSAMPLE']}ups_{params['COLOR_SPACE']}_{datetime_now}"

for fold, (idxT, idxV) in enumerate(skf.split(train_IP_set)):
    
    if f_resume:
        with open(f"{save_name_temp}_test_loc.testobj", 'rb') as f: 
            test_set_loc = pickle.load(f)               
        with open(f"{save_name_temp}_test_NoL.testobj", 'rb') as f: 
            test_set_NoL = pickle.load(f)               
        with open(f"{save_name_temp}_test_combo.testobj", 'rb') as f: 
            test_set_combo = pickle.load(f)               
    ...
    for n_ch, ch in enumerate(channels): # running the same model with inputs of just RGB, RGB+EI, RGB+MI, RGB+MI+EI for each fold
        ...
        # with tf.device('/GPU:1'):
            ...
            # models[ch] = build_model_simple(dim = params['IMG_SIZES'], n_ch = cur_num_channels, net_ind = params['EFF_NETS'], 
            #                          dropout = False, focal_loss = params['FOCAL_LOSS'], PrRec_metric = params['PR_REC_METRIC'])
            models[ch] = build_model(dim = params['IMG_SIZES'], n_ch = cur_num_channels, net_ind = params['EFF_NETS'], 
                                      dropout = False, focal_loss = params['FOCAL_LOSS'], PrRec_metric = params['PR_REC_METRIC'])
            ...
            trn_gen = DataGenerator(x_trn_ext, labels = y_trn_ext, batch_size = cur_batch_size, img_size = cur_img_size, 
                                    ...
                                    drop_luminosity = drop_luminosity_ch, sel_channels = ch, skin_fit = params['FIT_SKIN'])
            ...
            if fold in skip_folds:
                print_log(f'Skipped training for fold {fold}', [main_logger])
            else:
                sv = ModelCheckpoint(
                ...
                es = EarlyStopping(monitor = 'val_loss', patience = params['ES_PATIENCE'], verbose = 1, mode='auto')
                ...
                history = models[ch].fit(
                    ...
                    callbacks = [sv, es, csv_logger, tensorboard_callback, get_lr_callback_kaggle(cur_batch_size)], 
            ...
            test_gen =          test_set_loc.get_test_gen(cur_img_size, cur_color_space, ch, skin_fit = params['FIT_SKIN'])
            test_gen_Kaggle =   test_set_Kaggle.get_test_gen(cur_img_size, cur_color_space, ch, skin_fit = params['FIT_SKIN'])
            test_gen_NoL =      test_set_NoL.get_test_gen(cur_img_size, cur_color_space, ch, skin_fit = params['FIT_SKIN'])
            test_gen_combo =    test_set_combo.get_test_gen(cur_img_size, cur_color_space, ch, skin_fit = params['FIT_SKIN'])
            ...
            print_log('Predicting Combo Test without TTA...', [main_logger])
            pred = models[ch].predict(test_gen_combo, verbose = 1)[:len(test_combo_list_IDs)]  
            test_set_combo.predictions[ch][:, fold] += pred[:,0] 
            ...
            test_set_combo.get_last_metrics(ch, fold)
            ...
            with open(f"{save_name_temp}_test_loc.testobj", 'wb') as f: 
                pickle.dump(test_set_loc, f)               
            with open(f"{save_name_temp}_test_NoL.testobj", 'wb') as f: 
                pickle.dump(test_set_NoL, f)               
            with open(f"{save_name_temp}_test_combo.testobj", 'wb') as f: 
                pickle.dump(test_set_combo, f)               
...
for n_ch, ch in enumerate(channels): # running the same model with inputs of just RGB, RGB+EI, RGB+MI, RGB+MI+EI for each fold
    ...
    test_set_combo.get_avg_metrics(ch)
    cm = confusion_matrix(test_set_combo.labels, test_set_combo.predictions[ch].mean(axis=1) >= 0.5)
    # print_log(f'Confusion matrix for {ch}', [main_logger])
    print_log(cm, [main_logger])
    ...
    scaler = (np.array(test_set_combo.roc_aucs[ch]).max() - np.array(test_set_combo.roc_aucs[ch]).min()+1) 
    weights[ch]= (test_set_combo.roc_aucs[ch] - np.array(test_set_combo.roc_aucs[ch]).min()/2) / scaler  
    # weighted average between folds/models
    oof_preds_w[ch] = np.squeeze(test_set_combo.predictions[ch]).dot(weights[ch])
    op = MM_scaler.fit_transform(oof_preds_w[ch].reshape(-1, 1))
    auc_w = roc_auc_score(test_set_combo.labels, op)
    pr_Score , recall, f1, _ = precision_recall_fscore_support(test_set_combo.labels, op >= 0.5, average = None)
    prec, rec, thr = precision_recall_curve(test_set_combo.labels, op)
    auc_precision_recall = auc(rec, prec)
    AP_score = average_precision_score(test_set_combo.labels, op)
    ts = f'    Mean weighted over 5 folds for {ch} channels over combo test set, no TTA'
    print_log(f'{ts:<85}- ROC AUC = {auc_w:.4f}, PR ROC = {auc_precision_recall:.4f}, f1 = {f1[1]:0.4f}, precision = {pr_Score[1]:0.4f}, recall = {recall[1]:0.4f}, Average precision score = {AP_score:0.4f}', [main_logger, res_logger])
    cm = confusion_matrix(test_set_combo.labels, op >= 0.5)
    # print_log(f'Confusion matrix for {ch}, mean weighted', [main_logger])
    print_log(cm, [main_logger])
...
with open(f"{save_name}_test_NoL.testobj", 'wb') as f: 
    pickle.dump(test_set_NoL, f)               
with open(f"{save_name}_test_combo.testobj", 'wb') as f: 
    pickle.dump(test_set_combo, f)               
...
plot_mean_PR(test_set_combo.labels, test_set_combo.predictions, channels, model_descr = f'{nets_names[params["EFF_NETS"]]}-full-ISIC', to_dir = f'{model_logs}/{save_name}_ROC_PR_plots', channels_first=False)
...
df_mean_metrics_temp_combo = pd.DataFrame(columns = mean_metrics_cols, index = channels)

df_mean_metrics_temp_loc['uuid'] = params['UUID']
df_mean_metrics_temp_NoL['uuid'] = params['UUID']
df_mean_metrics_temp_combo['uuid'] = params['UUID']
for n_ch, ch in enumerate(channels):
    df_mean_metrics_temp_loc.loc[ch] = [datetime_now, nets_names[params["EFF_NETS"]], params["IMG_SIZES"], params["BATCH_SIZES"], 
                                        ...
                                        test_set_loc.precisions_mean[ch][0], test_set_loc.recalls_mean[ch][0], cur_uuid, drop_luminosity_ch]
    df_mean_metrics_temp_NoL.loc[ch] = [datetime_now, nets_names[params["EFF_NETS"]], params["IMG_SIZES"], params["BATCH_SIZES"], 
                                        ...
                                        test_set_NoL.precisions_mean[ch][0], test_set_NoL.recalls_mean[ch][0], cur_uuid, drop_luminosity_ch]
    df_mean_metrics_temp_combo.loc[ch] = [datetime_now, nets_names[params["EFF_NETS"]], params["IMG_SIZES"], params["BATCH_SIZES"], 
                                        ch, 'test_combo',
                                        test_set_combo.roc_aucs_mean[ch][0],   test_set_combo.pr_aucs_mean[ch][0], test_set_combo.f1s_mean[ch][0], 
                                        test_set_combo.precisions_mean[ch][0], test_set_combo.recalls_mean[ch][0], cur_uuid, drop_luminosity_ch]

df_mean_metrics = pd.read_csv(f'{data_path}/mean_metrics.csv')
df_mean_metrics = pd.concat([df_mean_metrics, df_mean_metrics_temp_NoL, df_mean_metrics_temp_loc, df_mean_metrics_temp_combo]).reset_index(drop = True)    
