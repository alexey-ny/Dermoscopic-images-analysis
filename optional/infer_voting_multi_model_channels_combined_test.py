...
mean_metrics = 'mean_metrics_inferred.csv'
...
compare_preds_local_test_name = 'compare_preds_combo_test.csv'
compare_probs_local_test_name = 'compare_probs_combo_test.csv'
...
main_logger = myLogger('main', f"{model_logs}{datetime_now}-combo-vote-difficulty-main.log")
res_logger = myLogger('result', f"{model_logs}{datetime_now}-combo-vote-difficulty-results.log", False)
# main_logger = myLogger('main', f"{model_logs}{datetime_now}-compare-skin-fit-main.log")
# res_logger = myLogger('result', f"{model_logs}{datetime_now}-compare-skin-fit-results.log", False)
...
try:
    ...
    df_mean_metrics = pd.DataFrame(columns = mean_metrics_cols)
...
if os.path.isfile(data_path + compare_preds_local_test_name):
    ...
    df_compare_preds_local_test.isic_id = df_combo_test.isic_id
    df_compare_preds_local_test.target = df_combo_test.target
...
if os.path.isfile(data_path + compare_probs_local_test_name):
    ...
    df_compare_probs_local_test.isic_id = df_combo_test.isic_id
    df_compare_probs_local_test.target = df_combo_test.target
    
df_mean_metrics_temp = pd.DataFrame(columns = mean_metrics_cols, index = channels)


y_test = df_combo_test['target']
    ...
    def __data_generation(self, list_IDs_temp, idxs = None):
        ...
        if self.labels is not None: # training phase
            ...
            for i, ID in enumerate(list_IDs_temp):
                if self.skin_fit:
                    X[i,] = fit_skin(read_img(self.img_dir+ID+".jpg", self.img_size, self.color_space, toAugment = True, 
            ...
            for i, ID in enumerate(list_IDs_temp):
                if self.skin_fit:
                    X[i,] = fit_skin(read_img(self.img_dir+ID+".jpg", self.img_size, self.color_space, toAugment = False, 
                                 drop_luminosity = self.drop_luminosity, sel_channels = self.sel_channels))
                else:
...
class TestSet():
    ...
    def __init__(self, list_IDs, labels = None, batch_size = 1, img_size = (512, 512, 1), channels = ['3', 'EI', 'MI', '5'],
        ...
        # self.predictions_sf =  {'3': np.zeros((len(list_IDs), n_folds)), 
        #                       'EI': np.zeros((len(list_IDs), n_folds)), 
        #                       'MI': np.zeros((len(list_IDs), n_folds)), 
        #                       '5': np.zeros((len(list_IDs), n_folds)), 
        #                       'RGB': np.zeros((len(list_IDs), n_folds))}
...
test_list_IDs = df_combo_test.isic_id
...
# for c_model in models_ISIC[:1]:
    ...
    log_file = [f for f in all_model_files if '.log' in f]
    if len(param_file) < 1:
        print_log(f'Skipping {c_model} - no saved parametres', [main_logger, res_logger])
        continue
    if len(log_file) < 1:
        print_log(f'Skipping {c_model} - no saved logs', [main_logger, res_logger])
        continue
    model_datetime_now = log_file[0][:8]
    ...
    for fold in range(n_folds):      
        ...
        for n_ch, ch in enumerate(channels): # running the same model with inputs of just RGB, RGB+EI, RGB+MI, RGB+MI+EI for each fold
            ...
            # with tf.device('/GPU:1'):
                ...
                pred = model.predict(test_gen, verbose = 1)[:len(test_list_IDs)]  
    ...
    for n_ch, ch in enumerate(channels): # running the same model with inputs of just RGB, RGB+EI, RGB+MI, RGB+MI+EI for each fold
        ...
        df_mean_metrics_temp['uuid'] = cur_uuid   
        df_mean_metrics_temp.loc[ch] = [model_datetime_now, nets_names[params["EFF_NETS"]], params["IMG_SIZES"], 
                                        params["BATCH_SIZES"], ch, 'test_loc',
                                        test_set_loc.roc_aucs_mean[ch][0], test_set_loc.pr_aucs_mean[ch][0], 
                                        test_set_loc.f1s_mean[ch][0], test_set_loc.precisions_mean[ch][0], 
                                        test_set_loc.recalls_mean[ch][0], cur_uuid, drop_luminosity_ch]
   
        print_log('\n', [main_logger, res_logger])

    df_mean_metrics = pd.concat([df_mean_metrics, df_mean_metrics_temp]).reset_index(drop = True)    
...
# df_mean_metrics = pd.read_csv(f'{data_path}/{mean_metrics}')
# df_mean_metrics = pd.concat([df_mean_metrics, df_mean_metrics_temp]).reset_index(drop = True)    
df_mean_metrics.to_csv(f'{data_path}/{mean_metrics}', index = False)
