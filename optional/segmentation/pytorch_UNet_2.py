...
org_files = [x.split('\\')[2] for x in glob.glob(IMG_PATH + '*')]
# org_files = [x.split('\\')[2][:-4] for x in glob.glob(IMG_PATH + '*')]
# org_files = [x.split('\\')[2] for x in glob.glob(IMG_PATH + '*')]
# org_files = [x.split('\\')[3] for x in glob.glob(IMG_PATH + '*')]
# msk_files = [x.split('\\')[2][:-4] for x in glob.glob(MSK_PATH + '*')]
msk_files = [x.split('\\')[2] for x in glob.glob(MSK_PATH + '*')]
# dd1 = set(org_files)- set(msk_files)
# dd2 = set(msk_files) - set(org_files)
...
transforms = A.Compose([
    ...
    # A.Normalize(p=1.0, max_pixel_value = 1, mean = (0.485, 0.456, 0.406), std = (0.229, 0.224, 0.225)),
    ToTensorV2(),
])
...
class SkinDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:        
            augmented = self.transforms(image = image / 255, mask = mask / 255)
        except Exception as err:
            print(f'{image.shape}, {mask.shape}, file - {self.df.iloc[idx, 0]}')    
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(train_loader, unit="batch") as tepoch:
            for data, target in tepoch:
                ...
                loss = train_loss(outputs, torch.squeeze(target))
                ...
                tepoch.set_postfix(loss = loss.item(), DICE = train_dice)
 
        #torch.save(model.state_dict(), f'{model_name}_{str(epoch)}_epoch.pt')
        val_mean_iou = compute_iou(model, val_loader, threshold=0.5)
        ...
        # print("Epoch [%d]" % (epoch))
        print(f"Mean loss on train: {np.array(losses).mean():.4f}, \nMean DICE on train: {np.array(train_iou).mean():.4f}, mean DICE on validation: {val_mean_iou:.4f}")
    ...
    with torch.no_grad():
        ...
        for i_step, (data, target) in enumerate(loader):
           ...
           # print("val_output:", outputs.shape)
            ...
            out_cut = np.copy(np.squeeze(outputs).data.cpu().numpy())
            # out_cut = np.copy(outputs.data.cpu().numpy())
...
test_iou = compute_iou(unet, test_dataloader, threshold=0.5)

# print("Epoch [%d]" % (epoch))
print(f"Mean DICE on test: {np.array(test_iou).mean():.4f}")
